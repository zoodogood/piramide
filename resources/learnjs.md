...
// Для получения данных из переменной достаточно её названия
let y = x;
...
```
Например, содержит имя, текст сообщения или ссылку на видео с котиками  
...
Другой пример:  
**_Если ужас сбивает столку, успокойтесь, мысленно разделите код ниже на логические элементы и попробуйте понять саму идею_**
```js
...
**Важно заметить, что кроме строки и числа может использоваться любая переменная:**  
`array[ i ]`
